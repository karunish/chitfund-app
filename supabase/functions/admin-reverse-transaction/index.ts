...
serve(async (req: Request) => {
  ...
  try {
    ...
    // 2. Get transaction ID from request body
    ...
    // 3. Call the RPC function to reverse and delete the transaction
    const { error: rpcError } = await supabaseAdmin.rpc('admin_reverse_transaction', {
    ...
    return new Response(JSON.stringify({ message: "Transaction reversed and deleted successfully" }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })
