...
serve(async (req: Request) => {
  ...
  try {
    ...
    // 2. Get transaction data from request body
    const { userId, type, amount, description, userFullName } = await req.json();
    if (!userId || !type || !amount || !userFullName) {
    ...
    // 3. Call the RPC function to execute the transaction
    ...
    return new Response(JSON.stringify({ message: "Transaction logged successfully" }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })
