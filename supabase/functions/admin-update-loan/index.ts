...
serve(async (req: Request) => {
  ...
  try {
    ...
    // 2. Get loan ID and updates from request body
    const { loanId, updates } = await req.json();
    if (!loanId || !updates) {
        return new Response(JSON.stringify({ error: 'Missing loanId or update data.' }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 400,
        });
    }
    ...
    return new Response(JSON.stringify({ message: "Loan updated successfully" }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })
